...
function addto(amnt) {
    ...
    document.getElementById("credit").textContent = `Change: ${credit}`
    initbuttons()
}
function hideall(){
   //alert("hideall works")
...
document.addEventListener('click', (event) => { 
   ...
   if((event.target.className == "background-dismiss" || event.target.nodeName == "HTML")&&event.target.nodeName != "BUTTON"){//event.target.nodeName != 'BUTTON' && event.target.nodeName != 'DIV' )) {
      //alert(event.target.className)
    hideall();
   }
})
var autoupdate = setInterval(getTabs, 3000);
alert("Notice: \nThis website is currently in testing, which may result in unintentional bugs and deletion. \nPlease bear with me for now.")
